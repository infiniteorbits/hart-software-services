 ...
 * QSPI_init()
 */
void QSPI_init(qspi_instance_t * this_qspi, addr_t addr)
{
 ...
 * QSPI_get_config()
 */
void QSPI_get_config(qspi_instance_t * this_qspi, qspi_config_t * config)
{
    ...
    config->xip_addr =
        (uint8_t) ((reg & CTRL_XIPADDR_MASK) >> CTRL_XIPADDR_SHIFT);
    config->sample =
        (uint8_t) ((reg & CTRL_SAMPLE_MASK) >> CTRL_SAMPLE_SHIFT);
}

/*******************************************************************************
 * QSPI_polled_transfer_block()
 */
void QSPI_polled_transfer_block(qspi_instance_t * this_qspi, uint8_t num_addr_bytes,
     const void *const tx_buffer, uint32_t tx_byte_size, const void *const rd_buffer,
     uint32_t rd_byte_size, uint8_t num_idle_cycles)
{
    ...
    if ((tx_byte_size <= 0xFFFFu) &&
        (num_addr_bytes <= 4u) && (num_idle_cycles <= 15u)) {
        ...
        reg_read = HAL_get_32bit_reg(this_qspi->base_address, CONTROL);
        skips |=
            (((reg_read & CTRL_QMODE12_MASK) ? 1u : 0u) <<
             FRAMES_QSPI_SHIFT);
        ...
        HAL_set_32bit_reg(this_qspi->base_address, FRAMES,
                          (uint32_t) skips);
    }
    ...
    if (rd_byte_size) {
        ...
        for (idx = 0u; idx < words; ++idx) {
            do {
                reg_read =
                    HAL_get_32bit_reg(this_qspi->base_address, STATUS);
            } while ((reg_read) & STATUS_RXFIFOEMPTY_MASK);

            buf32[idx] =
                HAL_get_32bit_reg(this_qspi->base_address, X4RECEIVE_DATA);
        }
        ...
        for (idx = l_idx; idx < rd_byte_size; ++idx) {
            ...
            buf8[idx] =
                HAL_get_32bit_reg(this_qspi->base_address, RECEIVE_DATA);
        }
        ...
        do {
            skips =
                (uint64_t) (((uint32_t)
                             (HAL_get_32bit_reg
                              (this_qspi->base_address, STATUS))
                             & (uint32_t) (STATUS_FLAGSX4_MASK))
                            ? (uint32_t) (HAL_get_32bit_reg
                                          (this_qspi->base_address,
                                           X4RECEIVE_DATA))
                            : (uint32_t) (HAL_get_32bit_reg
                                          (this_qspi->base_address,
                                           RECEIVE_DATA)));
 ...
 */
uint8_t QSPI_irq_transfer_block(qspi_instance_t * this_qspi, uint8_t num_addr_bytes,
     const void *const tx_buffer, uint32_t tx_byte_size, const void *const rd_buffer,
     uint32_t rd_byte_size, uint8_t num_idle_cycles)
{
    uint32_t idx;
 ...
 */
void QSPI_set_status_handler(qspi_status_handler_t handler)
{
...
void qspi_isr(qspi_instance_t * this_qspi)
{
    uint32_t idx;
   // static uint32_t empty = 0u;
    ...
    if (STATUS_RXAVAILABLE_MASK ==
        (uint32_t) (status & STATUS_RXAVAILABLE_MASK)) {
        ...
        for (idx = 0u; idx < words; ++idx) {
            while ((HAL_get_32bit_reg(this_qspi->base_address, STATUS)) &
    ...
    if (STATUS_RXDONE_MASK == (uint32_t) (status & STATUS_RXDONE_MASK)) {
        ...
        HAL_set_32bit_reg(this_qspi->base_address, INT_ENABLE,
                          ((uint32_t)
                           (HAL_get_32bit_reg
                            (this_qspi->base_address, INT_ENABLE))
                           & ~(uint32_t) (INT_ENABLE_RXDONE_MASK |
                                          INT_ENABLE_RXAVAILABLE_MASK)));
