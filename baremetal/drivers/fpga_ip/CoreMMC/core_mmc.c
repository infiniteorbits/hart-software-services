                               ...
                               const mmc_params_t *this_mmc_params)
{
    ...
    ret_status = get_hardware_config(this_mmc); /* If issue with HW config, stop now */
    ...
    if (MMC_TRANSFER_SUCCESS == ret_status) {
         ...
         */
        response_status =
            coremmc_cif_send_cmd(this_mmc->base_addr, RESET_ARG,
                ...
                if (MMC_TRANSFER_SUCCESS == ret_status) {
                    ...
                    if (MMC_INIT_SUCCESS == ret_status) {
                        ...
                        if (MMC_TRANSFER_SUCCESS == ret_status) {
                            ...
                            this_mmc->hw_sec_count = csd_reg[212 / 4];  /* Offsets defined in JESD84-A441 extended CSD */
                            this_mmc->hw_ext_csd_rev =
                                (uint8_t) (csd_reg[192 / 4] & 0xFFU);
                            this_mmc->hw_card_type =
                                (uint8_t) (csd_reg[196 / 4] & 0xFFU);
                                             ...
                                             uint32_t dst_addr)
{
    ...
    do {
        response_status =
            coremmc_cif_send_cmd(this_mmc->base_addr, RCA_VALUE,
     ...
     */
    response_status = coremmc_cif_send_cmd(this_mmc->base_addr, dst_addr,
                                                ...
                                                uint32_t timeout_ticks)
{
    ...
    HAL_ASSERT(NULL != this_mmc);
    HAL_ASSERT(0x0U != (uint8_t) timeout_ticks);

    /* Initialize interrupt state machine */
    this_mmc->status.int_state = MMC_ISR_SW_PARSE_CMD13;    /* Set the current CoreMMC interrupt state machine
                                                               state */
    this_mmc->status.no_of_blks = 1U;   /* Number of blocks to either send or received in current command */
    this_mmc->status.blk_addr = dst_addr;   /* Block to start read/write from */
    this_mmc->status.start_systick = current_timeout_ticks; /* Set at start- to enable timeout */
    this_mmc->status.timeout_value_ticks = timeout_ticks;   /* In systicks */
    this_mmc->status.err_state = MMC_CMD_PROCESSING;    /* Error report from interrupt */
    ...
    coremmc_send_mmc_cmd(this_mmc->base_addr, RCA_VALUE, MMC_CMD_13_SEND_STATUS,
                                            ...
                                            uint32_t timeout_ticks)
{
    ...
    /* Initialize interrupt state machine */
    this_mmc->status.int_state = MMC_ISR_MW_PARSE_CMD13;    /* Set the current CoreMMC interrupt state machine
                                                               state */
                                            ...
                                            uint32_t *dst_addr)
{
    ...
    if (READ_SEND_EXT_CSD == src_addr) {
        response_status =
            coremmc_cif_send_cmd(this_mmc->base_addr, 0U, MMC_CMD_8_SEND_EXT_CSD);
    } else {
        response_status = coremmc_cif_send_cmd(this_mmc->base_addr, src_addr,
    ...
    if (!(ISR_BISR_MBSIR_ERR & isr_errors)) {   /* Abort if any errors */
        ...
        read_err_isr =
            ISR_READ_ERR & HAL_get_8bit_reg(this_mmc->base_addr, ISR);
        if (NO_ERROR == read_err_isr) {
             ...
             */
            do {
                response_status = coremmc_cif_send_cmd(this_mmc->base_addr, RCA_VALUE,
                                               MMC_CMD_13_SEND_STATUS);
            } while (DEVICE_BUSY == response_status);
                                               ...
                                               uint32_t timeout_ticks)
{
    ...
    /* Initialize interrupt state machine */
    this_mmc->status.no_of_blks = 0x1U; /* Number of blocks to either send or received in current command */
...
static cif_response_t set_mmc_dwidth(const mmc_instance_t *this_mmc)
{
    ...
    response_status = coremmc_cif_send_cmd(this_mmc->base_addr,
                                   MMC_DW_CSD | ((uint32_t) this_mmc->
                                                 hw_mmc_dwidth << 8u),
                                   MMC_CMD_6_SWITCH);
    /*
...
static mmc_transfer_status_t get_hardware_config(mmc_instance_t *this_mmc)
{
    ...
    if (this_mmc->hw_maj_revision == 0x03U) {
        this_mmc->hw_maj_revision =
            HAL_get_8bit_reg(this_mmc->base_addr, VR_MAJ);
        this_mmc->hw_min_revision =
            HAL_get_8bit_reg(this_mmc->base_addr, VR_MIN);
    } else {
...
static void get_card_sector_size(mmc_instance_t *this_mmc)
{
    ...
    (void)coremmc_cif_send_cmd(this_mmc->base_addr, RCA_VALUE, MMC_CMD_9_SEND_CSD);
...
static cif_response_t set_mmc_clk_div(const mmc_instance_t *this_mmc)
{
    ...
    if (TRANSFER_IF_SUCCESS == response_status) {
        response_status =
            coremmc_cif_send_cmd(this_mmc->base_addr, MMC_HS_MODE, MMC_CMD_6_SWITCH);
    }
...
static mmc_transfer_status_t oper_cond_config(const mmc_instance_t *this_mmc)
{
    ...
    do {
        response_status =
            coremmc_cif_send_cmd(this_mmc->base_addr, OP_COND, MMC_CMD_1_SEND_OP_COND);
...
static mmc_transfer_status_t dev_addr_config(const mmc_instance_t *this_mmc)
{
    ...
    response_status =
        coremmc_cif_send_cmd(this_mmc->base_addr, STUFF_BITS, MMC_CMD_2_ALL_SEND_CID);
    if (TRANSFER_IF_SUCCESS == response_status) {
        response_status = coremmc_cif_send_cmd(this_mmc->base_addr, RCA_VALUE,
    ...
    if (ISR_BISR_MBSIR_ERR & isr_flags) {
        ...
        case MMC_ISR_MW_PARSE_CMD13:
            ...
            if ((READY_FOR_DATA & MMC_Response) == 0U) {    /* Not ready */
                /* Re-send as busy */
                coremmc_send_mmc_cmd(this_mmc->base_addr, RCA_VALUE,
                             MMC_CMD_13_SEND_STATUS, CHECK_IF_CMD_SENT_NO);
                break;
            } else if (CURRENT_STATE_TRAN != (CURRENT_STATE12_9_MASK & MMC_Response)) { /* Wrong state */
                coremmc_send_mmc_cmd(this_mmc->base_addr, RCA_VALUE,
                             MMC_CMD_13_SEND_STATUS, CHECK_IF_CMD_SENT_NO);
                break;
            ...
            coremmc_send_mmc_cmd(this_mmc->base_addr,
                         (uint32_t) (this_mmc->status.no_of_blks),
        ...
        case MMC_ISR_MW_PARSE_CMD23:
            ...
            MMC_Response = HAL_get_32bit_reg(this_mmc->base_addr, RR1);
            if (R1_ERR_RESP_MASK & HAL_get_32bit_reg(this_mmc->base_addr, RR1)) {   /* Error */
                this_mmc->status.int_state = MMC_ISR_ERROR;
            ...
            coremmc_send_mmc_cmd(this_mmc->base_addr, this_mmc->status.blk_addr,
        ...
        case MMC_ISR_MW_PARSE_CMD25:
            ...
            HAL_set_8bit_reg(this_mmc->base_addr, MBIMR, MBIMR_MASK_MULTI_WRITE);   /* Set required interrupts */
            HAL_set_8bit_reg_field(this_mmc->base_addr, MBCSR_MBWSTRT, SET_BIT);    /* Tell MMC to kick-off write */
        ...
        case MMC_ISR_SW_PARSE_CMD24:
             ...
             * when sending */
            /* tell CoreMMC to kick-off write */
            HAL_set_8bit_reg(this_mmc->base_addr, BIMR, BIMR_MASK_SINGLE_WRITE);    /* Set required interrupts */
            HAL_set_8bit_reg_field(this_mmc->base_addr, BCSR_BWSTRT, SET_BIT);  /* Tell MMC to kick-off write */
        ...
        case MMC_ISR_MR_PARSE_CMD23:
            ...
            HAL_set_8bit_reg(this_mmc->base_addr, MBIMR, MBIMR_MASK_MULTI_READ);    /* set required interrupts */
            HAL_set_8bit_reg_field(this_mmc->base_addr, MBCSR_MBRSTRT, SET_BIT);    /* tell MMC to expect read */
        ...
        case MMC_ISR_EXT_CSD_PARSE_CMD13:
            ...
            /* Set-up CoreMMC to expect read */
            HAL_set_8bit_reg(this_mmc->base_addr, BIMR, BIMR_MASK_SINGLE_READ); /* Set required interrupts */
            HAL_set_8bit_reg_field(this_mmc->base_addr, BCSR_BRSTRT, SET_BIT);  /* Tell MMC to expect read */
            ...
            if (MMC_ISR_SR_PARSE_CMD13 == this_mmc->status.int_state) {
                ...
                coremmc_send_mmc_cmd(this_mmc->base_addr, 0, MMC_CMD_8_SEND_EXT_CSD,
                             CHECK_IF_CMD_SENT_NO);
            }
