...
/**
  ...
  to the eMMC device. A value of this type is returned by `coremmc_cif_send_cmd()`.
 */
...
/**
  `coremmc_cif_send_cmd()` handles the interface level command and response
  ...
  @code
    ...
    response_status = coremmc_cif_send_cmd(BASE_ADDR, MMC_DW_CSD, MMC_CMD_SWITCH)

    while(DEVICE_BUSY == response_status)
    {
        response_status = coremmc_cif_send_cmd(BASE_ADDR,
                                       RCA_VALUE,
                                       MMC_CMD_SEND_STATUS)
    }
  @endcode
 */
    cif_response_t coremmc_cif_send_cmd(addr_t addr, uint32_t cmd_arg,
                                uint8_t cmd_type);

/**
  `coremmc_send_mmc_cmd()` transfers the MMC bus command and argument to the MMC device
  ...
  @endcode
 */
    void coremmc_send_mmc_cmd(addr_t addr, uint32_t cmd_arg, uint8_t cmd_type,
