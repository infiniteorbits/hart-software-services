 ...
 * @param ddr_type
 */
DDR_TRAINING_SM ddr_training_state = DDR_TRAINING_INIT;
static uint32_t ddr_setup(void)
{
...
//#ifdef SWEEP_DQ_DELAY
#if 0
                    uint8_t lane;
                    static uint32_t dly_firstpass=0xFF;
                    ...
                    if(ddr_type == LPDDR4)
                    {
                        ...
                        CFG_DDR_SGMII_PHY->rpc220.rpc220 = 0x0UL;
...
#if 0
    uint64_t size = 4U;
    result = MTC_test((0xFU), start_address, size, &result );
#endif
    return result;
}
#endif
    ...
    write_calibration_using_mtc(uint8_t number_of_lanes_to_calibrate)
{
    ...
    for (cal_data=0x00000U;cal_data<0xfffffU;cal_data=cal_data+0x11111U)
    {
                                                                ...
                                                                laneToTest++)
        {
            ...
            if(result == 0U) /* if passed for this lane */
            {
                ...
                num_lanes_passed++;
                #if 1
                                if (num_lanes_passed == number_of_lanes_to_calibrate)
                                {
                                    all_line_passed = 1;
                                }
                #endif
    ...
    if(result == 0U || all_line_passed == 1)
    {
...
static uint8_t ddr_manual_addcmd_refclk_offset(DDR_TYPE ddr_type, uint8_t * refclk_sweep_index)
{
#if 1 //def MANUAL_ADDCMD_TRAINIG
